...
function MainBoard({ role }) {
  ...
  useEffect(() => {
    api
      .get("/api/products")
      .then((res) => {
        const cpuProducts = res.data.filter((p) => p.category === "MAINBOARD");
    ...
    <div>
      <div className="page-container">
        ...
        <div className="product-list">
          <div className="category-title">메인보드</div>
          {products.map((p) => (
            <div key={p.id} className="product-card">
              ...
              <div className="product-price-container">
                ...
                <button onClick={() => addToCart(p.id, "MAINBOARD")}>
                  담기
                </button>
              </div>
            </div>
