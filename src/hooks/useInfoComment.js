...
export const useInfoComment = () => {
  ...
  const fetchComments = useCallback(async (boardId, page = 0, size = 10) => {
    try {
      const res = await api.get(
        `/api/infoboard/${boardId}/comments?page=${page}&size=${size}`
      );
  ...
  const fetchCommentCount = useCallback(async (boardId) => {
    try {
      const res = await api.get(`/api/infoboard/${boardId}/comments/count`);
  ...
  const createComment = useCallback(
    async (boardId, iCommentContent, parentId = null) => {
      try {
        await api.post(`/api/infoboard/${boardId}/comments`, {
          iCommentContent,
          parentId,
        });
  ...
  const updateComment = useCallback(
    async (boardId, commentId, iCommentContent) => {
      try {
        await api.put(`/api/infoboard/${boardId}/comments/${commentId}`, {
          iCommentContent,
        });
        await fetchComments(boardId, commentPage);
        return true;
  ...
  const deleteComment = useCallback(
    async (boardId, commentId) => {
      ...
      try {
        await api.delete(`/api/infoboard/${boardId}/comments/${commentId}`);
        await fetchComments(boardId, commentPage);
        await fetchCommentCount(boardId);
        alert("댓글이 삭제되었습니다.");
        return true;
