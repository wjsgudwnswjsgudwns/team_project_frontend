...
export default function CounselBottomPostList({
  currentPostId,
  initialPage = 0,
  onPostClick,
}) {
  ...
  const fetchNearbyPosts = async (pageNum) => {
    ...
    setIsLoading(true);
    try {
      const res = await api.get(`/api/counselboard?page=${pageNum}&size=10`);
          ...
          posts.map((post) => (
            ...
            >
              <div className="bottom-post-header">
                <h4 className="bottom-post-title-text">{post.ctitle}</h4>
