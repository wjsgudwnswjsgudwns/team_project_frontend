...
export default function InfoCommentItem({
  comment,
  currentUsername,
  depth = 0,
  onReplySubmit,
  ...
  const [editContent, setEditContent] = useState(comment.iCommentContent);
  ...
  const handleEditCancel = () => {
    setEditContent(comment.iCommentContent);
    setIsEditing(false);
  };
    ...
    >
      <div
        className={`comment-item ${comment.isAuthor ? "author-comment" : ""} ${
          comment.iCommentDeleted ? "deleted-comment" : ""
        }`}
      >
        <div className="comment-header">
          ...
          <span className="comment-date">
            {new Date(comment.iCommentWriteTime).toLocaleString("ko-KR")}
            {comment.iCommentUpdateTime && " (수정됨)"}
          </span>
        </div>
        ...
        {isEditing ? (
          ...
          <>
            <div className="comment-content">{comment.iCommentContent}</div>

            {!comment.iCommentDeleted && (
