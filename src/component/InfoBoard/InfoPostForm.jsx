...
export default function InfoPostForm({
  ...
  const [localTitle, setLocalTitle] = useState(formData.iTitle);
  const [showVideoModal, setShowVideoModal] = useState(false);
  const [videoUrl, setVideoUrl] = useState("");

  useEffect(() => {
    setLocalTitle(formData.iTitle);
  }, [formData.iTitle]);

  useEffect(() => {
    if (contentEditableRef.current) {
      if (contentEditableRef.current.innerHTML !== formData.iContent) {
        contentEditableRef.current.innerHTML = formData.iContent || "";
      }
    }
  }, [formData.iContent, isEditing]);
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (contentEditableRef.current) {
          ...
          onFormChange({
            iTitle: localTitle,
            iContent: contentEditableRef.current.innerHTML,
          });
        }
      } catch (error) {
  ...
  const handleTitleChange = (e) => {
    ...
    onFormChange({
      iTitle: newTitle,
      iContent: contentEditableRef.current?.innerHTML || formData.iContent,
    });
  };
