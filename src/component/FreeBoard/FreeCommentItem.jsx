...
export default function FreeCommentItem({
  comment,
  currentUsername,
  depth = 0,
  onReplySubmit,
  ...
  const [editContent, setEditContent] = useState(comment.fCommentContent);
  ...
  const handleEditCancel = () => {
    setEditContent(comment.fCommentContent);
    setIsEditing(false);
  };
    ...
    >
      <div
        className={`comment-item ${comment.isAuthor ? "author-comment" : ""} ${
          comment.fCommentDeleted ? "deleted-comment" : ""
        }`}
      >
        <div className="comment-header">
          ...
          <span className="comment-date">
            {new Date(comment.fCommentWriteTime).toLocaleString("ko-KR")}
            {comment.fCommentUpdateTime && " (수정됨)"}
          </span>
        </div>
        ...
        {isEditing ? (
          ...
          <>
            <div className="comment-content">{comment.fCommentContent}</div>

            {!comment.fCommentDeleted && (
